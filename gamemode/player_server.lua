...
local function DoMainThink( v )
	...
	if( not MYSQL_DED ) then
		...
		if( v:GetField( "charactercreate" ) == 0 ) then
			...
			if( v:GetField( "name" ) ~= v:Nick() and ( v:GetField( "newnick" ) == "" ) ) then
				...
				if( v:IsValid() ) then
					...
					if( v:GetField( "ignorenextname" ) ) then
						...
						if( v:GetNWInt( "tiedup" ) == 1 or v:GetField( "isko" ) == 1 ) then
						
							v:ConCommand( "setname " .. v:GetField( "name" ) );
							v:SetField( "name", v:Nick() );
							v:SetField( "ignorenextname", true );
							
						elseif( string.len( v:Nick() ) > 3 and v:SaveExists( v:Nick() ) ) then
							...
							v:NewData();
							v:SetField( "charactercreate", 1 );
							v:SetPrivateInt( "charactercreate", 1 );
...
local function DoStatusThink( v )
		...
		if( v:Alive() ) then
			...
			if( v:GetField( "isko" ) == 0 ) then
				...
				if( v:GetNWFloat( "conscious" ) >= 100 and v:GetField( "passedout" ) == 0 ) then
...
function GM:PlayerInitialSpawn( ply )
	...
	local function SendGlobalData( ply )
	
		for k, v in pairs( TS.GlobalInts ) do
		
			umsg.Start( "SetGlobalInt", ply );
				umsg.String( k );
				umsg.Long( v );
			umsg.End();
		...
		for k, v in pairs( TS.GlobalFloats ) do
		
			umsg.Start( "SetGlobalFloat", ply );
				umsg.String( k );
				umsg.Float( v );
			umsg.End();
		...
		for k, v in pairs( TS.GlobalStrings ) do
		
			umsg.Start( "SetGlobalString", ply );
				umsg.String( k );
				umsg.String( v );
			umsg.End();
	...
	timer.Simple( .1, SendGlobalData, ply );
	...
	ply:SetField( "gamename", ply:GetFriendsNick() );
	
	TS.Log( "server_joins.txt", ply:GetFriendsNick() .. ply:LogInfo() .. " entered game" );
	...
	ply:SetPrivateString( "job", "Unemployed" );
	ply:SetPrivateFloat( "inventory.MaxSize", 4 );
	...
	if( ply:GetField( "uid" ) ~= 0 ) then
		...
		elseif( ply:SaveExists( ply:Nick() ) ) then
			...
			timer.Simple( 3, DoCharacterCreate, ply );
...
--	else
	...
	timer.Simple( 2, TS.SendClientTeamData, ply );
	...
	timer.Simple( 1, ply.DoUmsg, ply, "CreateChatVGUI" );
	...
	for k, v in pairs( TS.FactoryItems ) do
		...
		local function AddStoreItem( ply, v )
		...
		timer.Simple( 1 + delay, AddStoreItem, ply, v );
	...
	if( MYSQL_DED ) then
		...
		local mdl = TS.Models[math.random( 1, #TS.Models )];
...
function TS.UpdateDoorOwnership( steamid )
	...
	local tbl = sql.Query( "SELECT `id` FROM `rtrp_doors` WHERE `SteamID` = '" .. steamid .. "'" );
	...
	if( not inserver ) then
		...
		sql.Query( "DELETE FROM `rtrp_doors` WHERE `SteamID` = '" .. steamid .. "'" );
...
function GM:PlayerDisconnected( ply )
	...
	if( ply:GetField( "uid" ) ) then
		local	query = "UPDATE `rtrp_users` SET `serverIP` = '', `serverPort` = '' WHERE `uID` = '" .. ply:GetField( "uid" ) .. "'";	
